...
def run_x8_url(url, wordlist, method, header, max_param, body, additional_cmd):
        
        command = ["x8", "--mimic-browser", "--append"]
        command.extend(["--url", url])
        command.extend(["--wordlist", str(wordlist)])
        command.extend(["--max", str(max_param)])
        command.extend(["--output", f"{url.split("/")[2].split(":")[0]}-hparam.txt"])
...
def run_x8_raw(request, wordlist, max_param, additional_cmd):
        
        command = ["x8", "--append"]
        command.extend(["--request", str(request)])
        command.extend(["--wordlist", str(wordlist)])
        command.extend(["--max", str(max_param)])
        command.extend(["--output", f"{str(request).split("/")[-1].split('.')[0]}-hparam.txt"])
        if additional_cmd:
                command.extend(additional_cmd.split())
