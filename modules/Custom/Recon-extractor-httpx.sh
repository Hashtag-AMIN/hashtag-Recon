...
if [ $# -ne 1 ]
  then
    ...
    echo "Usage: bash Recon-extractor-httpx.sh <file(subdomain-file.txt)>"
    exit
fi
...
grep -hE "(20[0-8]{1})\]" $1 | cut -d " " -f1 | sort -u > $file_name.20X.txt
grep -hE "\[(30[0-8]{1})" $1 | cut -d " " -f1 | sort -u > $file_name.30X.txt
grep -hE "(4[0-2]{1}[0-9]{1})\]" $1 | cut -d " " -f1 | sort -u > $file_name.4XX.txt
grep -hE "(5[0-1]{1}[0-9]{1})\]" $1 | cut -d " " -f1 | sort -u > $file_name.50X.txt
