...
if [ $# -ne 2 ]
  then
    ...
    echo "Usage: bash Recon-E-dns-Brute-heavy.sh <Domain(example.com)> <SubdomainList(example.com.resolve.txt)>"
    exit
fi
...
sort -u $2 $1.dnsBrute-1st.txt $1.dnsBrute-2nd.txt | dnsgen --wordlist ./wordlist/dns-dnsgen-wordlist-heavy.txt - > $1.dnsgen.txt
